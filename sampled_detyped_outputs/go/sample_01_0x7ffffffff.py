...
class Square:
    ...
    def set_neighbours(self: Square) -> None:
        ...
        for (dx, dy) in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            newx = box(int64(int64(x) + int64(dx)))
            newy = box(int64(int64(y) + int64(dy)))
            if 0 <= int64(newx) < 9 and 0 <= int64(newy) < 9:
    ...
    def move(self: Square, color: int64) -> None:
        ...
        for neighbour in self.neighbours:
            ...
            if neighcolor == 0:
                ...
                neighbour_ref: Square = cast(Square, neighbour.find(cast(bool, True)))
    ...
    def remove(self: Square, reference: Square, update: bool) -> None:
        ...
        for neighbour in self.neighbours:
            if neighbour.color != 0 and cbool(neighbour.removestamp != TIMESTAMP):
                neighbour_ref: Square = cast(Square, neighbour.find(cast(bool, update)))
    ...
    def find(self: Square, _update):
        ...
        if cast(Square, reference).pos != self.pos:
            reference = cast(Square, cast(Square, reference).find(cast(bool, update)))
            if update:
                self.reference = cast(Square, reference)
        return cast(Square, reference)
...
class EmptySet:
    ...
    def random_choice(self):
        ...
        while int64(choices):
            i = box(int64(int(random.random() * box(int64(choices)))))
            pos = self.empties[int64(i)]
            if cast(int, self.board.useful(box(int64(pos)))):
                return box(int64(pos))
            choices -= 1
            self.set(box(int64(i)), box(int64(self.empties[int64(choices)])))
...
class Board:
    ...
    def useful(self, _pos):
        ...
        TIMESTAMP += 1
        square = self.squares[pos]
        if cast(bool, self.useful_fast(cast(Square, square))):
            return True
        ...
        for neighbour in square.neighbours:
            ...
            if neighcolor == 0:
                empties += 1
                continue
            neighbour_ref = cast(Square, neighbour.find(cast(bool, False)))
        ...
        for square in self.squares:
            ...
            for member in members1:
                for neighbour in member.neighbours:
                    if neighbour.color == 0:
                        ledges1 += 1
            root = cast(Square, square.find(cast(bool, False)))
            members2 = set()
            for square2 in self.squares:
                if square2.color != 0 and cbool(cast(Square, square2.find(cast(bool, False))) == root):
...
class UCTNode:
    ...
    def play(self, _board) -> None:
        ...
        ' uct tree search '
        color = cast(int, box(board.color))
        node = cast(UCTNode, self)
        path = CheckedList[UCTNode](CheckedList[UCTNode]([cast(UCTNode, node)]))
        pos = box(int64(0))
        while True:
            pos = box(int64(cast(UCTNode, node).select(cast(Board, board))))
            if int64(pos) == -1:
                break
            board.move(int64(pos))
            child = cast(UCTNode, node).pos_child[int64(pos)]
            if not child:
                child = cast(UCTNode, node).pos_child[int64(pos)] = UCTNode()
                child.unexplored = CheckedList[int](board.useful_moves())
                child.pos = int64(pos)
                child.parent = cast(UCTNode, node)
                CheckedList[UCTNode](path).append(child)
                break
            CheckedList[UCTNode](path).append(child)
            node = cast(UCTNode, child)
    ...
    def update_path(self, _board, _color, _path) -> None:
        ...
        for node in path:
            ...
            if node.parent is not None:
                mypar = node.parent
                bc = cast(None | UCTNode, mypar.best_child())
                if node.parent is not None:
...
def computer_move(_board):
    ...
    for game in range(GAMES):
        node = tree
        nboard.reset()
        nboard.replay(cast(Array[int64], ahist))
        node.play(cast(Board, nboard))
