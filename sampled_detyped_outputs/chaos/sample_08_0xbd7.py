    ...
    def __call__(self, _u):
        ...
        for ik in range(1, self.degree + 1):
            for ii in range(I - self.degree + ik + 1, I + 2):
                ...
                d[index] = cast(GVector, d[index].linear_combination(cast(GVector, d[index + 1]), float(co1), float(co2)))
        return d[0]
...
class Chaosgame(object):
    ...
    def __init__(self, splines: List[Spline], thickness: float, w: int, h: int, n: int) -> None:
        ...
        for spl in splines:
            ...
            for i in range(1, 1000):
                ...
                t = 1 / 999 * i
                curr = spl(t)
                length += float(curr.dist(cast(GVector, last)))
    ...
    def transform_point(self, point: GVector) -> GVector:
        ...
        if float(derivative.Mag()) != 0:
            basepoint.x += derivative.y / float(derivative.Mag()) * (y - 0.5) * self.thickness
            basepoint.y += -derivative.x / float(derivative.Mag()) * (y - 0.5) * self.thickness
        else:
            print('r', end='')
