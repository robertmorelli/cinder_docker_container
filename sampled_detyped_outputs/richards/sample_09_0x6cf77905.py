...
class Task(TaskState):
    ...
    def release(self, i: int64) -> Task:
        t: Task = cast(Task, Task.findtcb(self, box(i)))
    ...
    def qpkt(self, pkt: Packet) -> Task:
        t: Task = cast(Task, Task.findtcb(self, box(int64(pkt.ident))))
        taskWorkArea.qpktCount += 1
        pkt.link = None
        pkt.ident = self.ident
        return cast(Task, t.addPacket(cast(Packet, pkt), cast(Task, self)))
    ...
    def fn(self, pkt: Optional[Packet], r: TaskRec) -> Task:
        ...
        if pkt is not None:
            if pkt.kind == int64(K_WORK):
                ...
                cast(Optional[Packet], h.deviceInAdd(cast(Packet, pkt)))
