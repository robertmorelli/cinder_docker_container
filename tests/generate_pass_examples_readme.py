...
def build_readme(cases: list[ExampleCase], script_rel_path: str) -> str:
    ...
    lines.append("")
    lines.append("Each transform is shown as:")
    lines.append("- input (`*.from.py`)")
    lines.append("- expected output (`*.to.py`)")
    lines.append("- optional function-level plan (`*.plan.txt`)")
    lines.append("")
    lines.append("## Cases")
    lines.append("")
    for case in cases:
        lines.append(f"- [{case.name}](#{anchor_for_case(case.name)})")
    lines.append("")
    ...
    for case in cases:
        ...
        if case.plan_file is not None:
            lines.append(f"Plan (`{case.plan_file.name}`):")
            lines.append("```text")
            lines.append(read_text(case.plan_file))
            lines.append("```")
            lines.append("")

        lines.append(f"Input (`{case.from_file.name}`):")
        lines.append("```python")
        lines.append(read_text(case.from_file))
        lines.append("```")
        lines.append("")

        lines.append(f"Expected Output (`{case.to_file.name}`):")
        lines.append("```python")
        lines.append(read_text(case.to_file))
        lines.append("```")
        lines.append("")
